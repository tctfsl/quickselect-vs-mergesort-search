...
int quickSelect(IntPtr& arr, int firstIndex, int lastIndex, int target, int size, int counter, int pivotType) {
  if(firstIndex <= lastIndex) {
    int pivotIndex = partition(arr, firstIndex, lastIndex, pivotType);   // To get the index of pivot from the partition.
    
    if(size <= 20) {
      printInfo(arr, size, firstIndex, lastIndex, counter, pivotIndex);  // Print out the table when size of array is less than 21
      counter--;
    }
    
    if(target == pivotIndex) {
      return arr[pivotIndex];    // If target is equal to pivot index, return back the arr[pivot].
    }
    
    if(target < pivotIndex) {
      return quickSelect(arr, firstIndex, (pivotIndex - 1), target, size, counter, pivotType);
    }
    else {
      return quickSelect(arr, (pivotIndex + 1), lastIndex, target, size, counter, pivotType);
    }
  }
}